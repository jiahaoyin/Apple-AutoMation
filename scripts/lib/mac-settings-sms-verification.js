...
export async function completeSupervisedMacSettingsSmsVerification(options = {}) {
  ...
  const reportStage = (stage, detail = "") => {
    if (stage !== lastStage || pollCount === 0) {
      const messages = {
        phone_selection: `[短信] 选择号码尾号 **${suffix}`,
        code_entry: "[短信] 等待验证码",
        waiting: "[短信] 等待验证界面加载",
        manual_phone: "[短信] 未自动完成号码选择",
        manual_code: "[短信] 未自动获取验证码",
      };
      const msg = messages[stage] || `[短信] ${stage}`;
  ...
  while (readRemainingMs(deadline, now) > 0) {
    ...
    if (!state.ok || state.stage === "invalid") {
      console.warn("[短信] 无法读取验证界面，系统设置是否可见？");
    ...
    if (state.stage === "waiting" && ocrAvailable) {
      ...
      if (axWaitingCount >= 8) {
        axWaitingCount = 0;
        console.log("[短信] 尝试 OCR 辅助检测界面状态 …");
        try {
          ...
          if (ocrState?.ok && ocrState.stage === "code_entry") {
            console.log("[短信] OCR 检测到验证码输入界面 (尾号 " + (ocrState.suffix || "?") + ")");
            ...
            console.log("[短信] OCR 检测到号码选择界面");
            // Fall through to phone_selection handling below
            state = { ok: true, stage: "phone_selection" };
            waitingStartedAt = 0;
          } else {
            console.log("[短信] OCR 未能确定界面状态: " + (ocrState?.message || "unknown"));
          }
        } catch {
          console.log("[短信] OCR 调用失败，继续使用 AX 检测");
        }
      }
    }
    ...
    if (state.stage === "phone_selection" && !selectionSubmitted) {
      ...
      if (!selectionSubmitted) {
        ...
        console.warn(`\n[短信] ⚠  号码自动选择未完成`);
        console.warn(`[短信]    请在系统设置中手动选择尾号为 **${suffix} 的号码`);
        console.warn("[短信]    并点击「继续」，完成后按回车…\n");
        ...
        try {
          await promptForHiddenVerificationCode({
            prompt: `[短信] 选择尾号 **${suffix} 并点击继续后，按回车确认`,
            timeoutMs: readRemainingMs(manualDeadline, now),
            allowEmpty: true,
          });
          console.log("[短信] 已确认手动选号，继续流程…");
          selectionSubmitted = true;
        } catch {
          console.warn("[短信] 手动选号等待超时，将重试…");
        }
        continue;
      }
    }
    ...
    if (state.stage === "code_entry") {
      ...
      let code = null;
      if (codeProvider) {
        console.log("[短信] 正在通过短信服务获取验证码 …");
        code = await acquireCode(codeProvider, providerTimeoutMs);
        if (code) {
          console.log("[短信] ✓ 已从短信服务获取验证码");
        }
      }
      ...
      if (!code) {
        ...
        if (isTTY) {
          ...
          console.warn("[短信] 请在终端输入系统设置中显示的 6 位验证码");
          console.warn("[短信] 输入时不会回显");
          try {
        ...
        if (!code) {
          console.warn("[短信] 未输入验证码，将重试…");
          await pause(Math.min(pollIntervalMs, readRemainingMs(deadline, now)));
          continue;
        }
        console.log("[短信] ✓ 已接受手动输入的验证码");
      }
      ...
      if (filled?.ok !== true && ocrAvailable) {
        console.log("[短信] AX 填写失败，尝试 OCR 辅助键入 …");
        try {
          ...
          if (ocrCodeResult?.ok && ocrCodeResult.stage === "code_submitted") {
            console.log("[短信] ✓ OCR 已键入验证码");
            filled = { ok: true, stage: "code_submitted" };
          }
        } catch {
          console.warn("[短信] OCR 键入失败");
        }
      }
      if (filled?.ok !== true) {
        console.warn("[短信] 自动填写验证码失败，请直接在系统设置中输入后手动提交");
      } else {
        console.log("[短信] ✓ 验证码已提交");
      }
    ...
    if (state.stage === "waiting") {
      ...
      if (now() - lastWaitProgressAt >= WAIT_PROGRESS_INTERVAL_MS) {
        ...
        console.log(`[短信] 仍在等待 … (已等待 ${elapsedSec} 秒，每 ${pollIntervalMs}ms 检测一次)`);
        lastWaitProgressAt = now();
      }
      ...
      if (waitedMs >= MAX_WAIT_WITHOUT_PROGRESS_MS) {
        console.warn(`\n[短信] ⚠  已等待 ${Math.round(waitedMs / 1000)} 秒仍未检测到短信验证界面`);
        console.warn("[短信]    如果验证码界面已显示，可直接在下方输入验证码");
        console.warn("[短信]    如果号码选择界面已显示，请手动选号并点继续后输入验证码\n");

        if (isTTY) {
          try {
            const manualCode = await acquireCode(manualCodeProvider, manualTimeoutMs);
            if (manualCode) {
              console.log("[短信] ✓ 已接受手动验证码，正在提交 …");
              ...
              if (filled?.ok !== true) {
                console.warn("[短信] 自动填写失败，请在系统设置中直接输入验证码后手动提交");
              } else {
                console.log("[短信] ✓ 验证码已提交");
              }
              return { status: "submitted" };
